...
class MenuItemView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not menu_id:
            # If menu_id is not provided, return all menu items for the given store_id
            try:
                restaurant = Restaurant.objects.get(store_id=store_id)
                all_menu_items = MenuItem.objects.filter(menu__restaurant=restaurant).select_related('menu')
                response_data = [
                    {
                        "menu_id": item.menu.id,
                        "menu_name": item.menu.category_name,
